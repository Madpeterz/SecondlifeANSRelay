...
foreach ($relayTargets as $relay) {
    try {
        error_log("sending to " . $url, 3, '/proc/self/fd/2');
        $relayUri = $url . "?" . $_SERVER['QUERY_STRING'];
        $request = new Request("GET", $relayUri, $headers, "");
        $promises[] = $client->getAsync($request);
    } catch (Exception $e) {
        error_log("Failed to write to a relay target " . $relay . " because: " . $e->getMessage(), 3, '/proc/self/fd/2');
    }
}
...
while ($done < $waiting) {
    ...
    error_log("tick(" . $delta . ")", 3, '/proc/self/fd/2');
    $last = $now;
    $handler->tick();
}
